...
Flat script. Every panel has its own axes handle:
	...
	axa axb axc   gs[0, 0:2] [2:4] [4:6]   conversion C, three windows
	axd axe axf   gs[1, 0:2] [2:4] [4:6]   |F_bc| with propagation vectors
	axg           gs[2, 0:4]               shelf break C time series
	axh           gs[2, 4:6]               shelf budget
	axi           gs[3, 4:6] or shared     slope budget
...
C_LIM   = 2.0e-2      # W m-2, half range of the conversion colour scale
F_MAX   = 5.0         # kW m-1, top of the flux colour scale
VEC_STEP = 7          # quiver subsampling, every Nth grid point
VEC_SCALE = 60        # quiver scale, larger means shorter arrows
TICK_FS = 7
TOP_WSPACE = 0.05
CBAR_IN_EACH = True
...
NOISE = float(ds.attrs.get('ctrl_fortnightly_sd', np.nan))
...
def dress(ax, labels_left=True, labels_bottom=True):
	...
	ax.contour(lon, lat, h, levels=[50, 1000, 2000], colors='0.45',
...
def cbar_on_land(ax, mappable, label, extend='both',
				 anchor=(0.05, 0.06, 1, 1), width='42%', height='3.0%'):
...
axg.legend(loc='upper left', ncol=4, framealpha=0.9)
